...
#Factors
xy_factor = 0x7FFF/xy_lim # bits/cm
z_factor = 0xAAAA/z_lim_up # bits/cm
gamma_factor = 0x7F/gamma_lim # bits/°
...
def buildMSG():
    ...
    pxB = 1<<15 | int(round(abs(Px)*xy_factor)) if Px<0 else int(round(Px*xy_factor))
    pyB = 1<<15 | int(round(abs(Py)*xy_factor)) if Py<0 else int(round(Py*xy_factor))
    pzB = 0xAAAA + int(round(abs(Pz)*z_factor)) if Pz<0 else int(round(Pz*z_factor))
